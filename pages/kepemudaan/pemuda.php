                        ...
                        <div class="header">
                            <?php 
								if(isset($_POST['cari'])){
									$dari = $_POST['dari'];
									$sampai = $_POST['sampai'];
									$sqlkat = "SELECT pemuda.*, tahun.* 
											   FROM pemuda, tahun WHERE 
											   pemuda.id_tahun=tahun.id_tahun AND 
											   tahun.tahun='$sampai'";
									$querykat = mysqli_query($koneksidb,$sqlkat);
									$tahun = mysqli_fetch_array($querykat);
							?>
							<h2>
                                Data Jumlah Pemuda di Kabupaten Tasikmalaya Tahun <?php echo $tahun['tahun']; ?>
                            </h2>
							<?php }else{ ?>
							<h2>
                                Data Jumlah Pemuda di Kabupaten Tasikmalaya
                            </h2>
							<?php }?>
                        </div>
                        <div class="body">
								...
								<table class="display table table-striped table-bordered table-hover" width="100%" id="example-table" cellspacing="0">
									<thead>
										<tr>
											...
											<th class="text-center" width="100px">Act</th>
										</tr>
									</thead>
									<tbody>
										<?php 
											...
											if(isset($_POST['cari'])){
												$dari = $_POST['dari'];
												$sampai = $_POST['sampai'];
												$sqlthn = "SELECT pemuda.*, pendidikan.*, tahun.* FROM pemuda, pendidikan, tahun WHERE 
														   pemuda.id_pendidikan=pendidikan.id_pendidikan AND 
														   pemuda.id_tahun=tahun.id_tahun AND 
														   tahun.tahun='$sampai' ORDER BY pemuda.id_pendidikan ASC";
											}else{
											$sqlthn = "SELECT pemuda.*, pendidikan.*, tahun.* FROM pemuda, pendidikan, tahun WHERE 
													   pemuda.id_pendidikan=pendidikan.id_pendidikan AND
													   pemuda.id_tahun=tahun.id_tahun AND 
											...
											while ($result = mysqli_fetch_array($querythn)){
													$total += $result['laki_laki'];
													$total2 += $result['perempuan'];
													$total3 = $result['laki_laki'] + $result['perempuan'];
													$total4 += $total3;
													$nomor++;
										?>
										<tr>
											...
											<td class="text-center"><?php echo htmlentities(number_format($result['laki_laki'], 0, ",", "."));?></td>
											<td class="text-center"><?php echo htmlentities(number_format($result['perempuan'], 0, ",", "."));?></td>
											<td class="text-center"><?php echo htmlentities(number_format($total3, 0, ",", "."));?></td>
											<td class="text-center">
												<button type="button" class="btn btn-warning btn-circle waves-effect waves-circle waves-float" 
														data-toggle="modal" data-target="#editModal<?php echo htmlentities($result['id_pemuda']);?>">
													<i class="material-icons">mode_edit</i>
												</button>&nbsp;&nbsp;
												<!-- Large Size -->
												<div class="modal fade" id="editModal<?php echo htmlentities($result['id_pemuda']);?>" tabindex="-1" role="dialog">
													<div class="modal-dialog modal-lg" role="document">
														<div class="modal-content">
															<div class="modal-header">
																<h4 class="modal-title" id="editModalLabel">Form Edit Data pemuda</h4>
															</div>
															<div class="modal-body">
																<form method="POST" action="module=pemuda-edit">
																	<div class="col-sm-6">
																		<div class="form-group">
																			<label class="form-control-label" style="float:left;"><b>Pendidikan</b><span style="color:red">*</span></label>
																			<div style="text-align:left;">
																				<select class="form-control pendidikan" name="pendidikan" style="width:100%;" required>
																					<option value="<?php echo htmlentities($result['id_pendidikan']);?>"><?php echo htmlentities($result['pendidikan']);?></option>
																					<?php
																						$mySql = "SELECT * FROM pendidikan ORDER BY id_pendidikan";
																						$myQry = mysqli_query($koneksidb, $mySql);
																						while ($myData = mysqli_fetch_array($myQry)) {
																	...
																	<div class="col-sm-6">
																		<div class="form-group">
																			<label class="form-control-label" style="float:left;"><b>Jumlah Pemuda Laki-laki</b></label>
																			<div class="form-line">
																				<input type="hidden" name="id" value="<?php echo htmlentities($result['id_pemuda']);?>">
																				<input type="number" name="laki_laki" class="form-control" min=0 value="<?php echo $result['laki_laki'];?>">
																			</div>
																		</div>
																	</div>
																	<div class="col-sm-6">
																		<div class="form-group">
																			<label class="form-control-label" style="float:left;"><b>Jumlah Pemuda Perempuan</b></label>
																			<div class="form-line">
																				<input type="number" name="perempuan" class="form-control" min=0 value="<?php echo $result['perempuan'];?>">
																			</div>
																		</div>
																	</div>
															</div>
												...
												<button type="button" class="btn btn-danger btn-circle waves-effect waves-circle waves-float"
														onclick="Swal.fire({
															...
															confirmButtonText: 'Ya Hapus',
																...
																window.location = 'pemuda-del.php?id=<?php echo htmlentities($result['id_pemuda']);?>';
																}
															})">
