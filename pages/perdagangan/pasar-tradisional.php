...
<body class="theme-red">
                        ...
                        <div class="header">
                            <?php 
								if(isset($_POST['cari'])){
									$dari = $_POST['dari'];
									$sampai = $_POST['sampai'];
									$sqlkat = "SELECT pasar_tradisional.*, tahun.* 
											   FROM pasar_tradisional, tahun WHERE 
											   pasar_tradisional.id_tahun=tahun.id_tahun AND 
											   tahun.tahun='$sampai'";
									$querykat = mysqli_query($koneksidb,$sqlkat);
									$tahun = mysqli_fetch_array($querykat);
							?>
							<h2>
                                Data Jumlah Pasar Tradisional di Kabupaten Tasikmalaya Tahun <?php echo $tahun['tahun']; ?>
                            </h2>
							<?php }else{ ?>
							<h2>
                                Data Jumlah Pasar Tradisional di Kabupaten Tasikmalaya
                            </h2>
							<?php }?>
                        </div>
                        <div class="body">
								...
								<table class="display table table-striped table-bordered table-hover" width="100%" id="example-table" cellspacing="0">
									<thead>
										<tr>
											...
											<th class="text-center">Permanen (Unit)</th>
											<th class="text-center">Semi Permanen (Unit)</th>
											<th class="text-center">Tanpa Bangunan (Unit)</th>
									...
									<tbody>
										<?php 
											...
											if(isset($_POST['cari'])){
												$dari = $_POST['dari'];
												$sampai = $_POST['sampai'];
												$sqlthn = "SELECT pasar_tradisional.*, kecamatan.*, tahun.* FROM pasar_tradisional, kecamatan, tahun WHERE 
														   pasar_tradisional.id_kecamatan=kecamatan.id_kecamatan AND 
														   pasar_tradisional.id_tahun=tahun.id_tahun AND 
											...
											while ($result = mysqli_fetch_array($querythn)){
													$total += $result['permanen'];
													$total2 += $result['semi'];
													$total3 += $result['tanpa_bng'];
													$nomor++;
										?>
										<tr>
											<td class="text-center"><?php echo $nomor;?></td>
											<td><?php echo $result['kecamatan'];?></td>
											<td class="text-center"><?php echo htmlentities(number_format($result['permanen'], 0, ",", "."));?></td>
											<td class="text-center"><?php echo htmlentities(number_format($result['semi'], 0, ",", "."));?></td>
											<td class="text-center"><?php echo htmlentities(number_format($result['tanpa_bng'], 0, ",", "."));?></td>
											<td class="text-center">
												<button type="button" class="btn btn-warning btn-circle waves-effect waves-circle waves-float" 
														data-toggle="modal" data-target="#editModal<?php echo htmlentities($result['id_pasar_tradisional']);?>">
													<i class="material-icons">mode_edit</i>
												</button>&nbsp;&nbsp;
												<!-- Large Size -->
												<div class="modal fade" id="editModal<?php echo htmlentities($result['id_pasar_tradisional']);?>" tabindex="-1" role="dialog">
													<div class="modal-dialog modal-lg" role="document">
														<div class="modal-content">
															<div class="modal-header">
																<h4 class="modal-title" id="editModalLabel">Form Edit Data Pasar Tradisional</h4>
															</div>
															<div class="modal-body">
																<form method="POST" action="module=pasar-tradisional-edit">
																	<div class="col-sm-6">
																		<div class="form-group">
																	...
																	<div class="col-sm-4">
																		<div class="form-group">
																			<label class="form-control-label" style="float:left;"><b>Jumlah Pasar Permanen</b></label>
																			<div class="form-line">
																				<input type="hidden" name="id" value="<?php echo htmlentities($result['id_pasar_tradisional']);?>">
																				<input type="number" name="permanen" class="form-control" min=0 value="<?php echo $result['permanen'];?>">
																			</div>
																		</div>
																	</div>
																	<div class="col-sm-4">
																		<div class="form-group">
																			<label class="form-control-label" style="float:left;"><b>Jumlah Pasar Semi Permanen</b></label>
																			<div class="form-line">
																				<input type="number" name="semi" class="form-control" min=0 value="<?php echo $result['semi'];?>">
																			</div>
																		</div>
																	</div>
																	<div class="col-sm-4">
																		<div class="form-group">
																			<label class="form-control-label" style="float:left;"><b>Jumlah Pasar Tanpa Bangunan</b></label>
																			<div class="form-line">
																				<input type="number" name="tanpa_bng" class="form-control" min=0 value="<?php echo $result['tanpa_bng'];?>">
																			</div>
																		</div>
																	</div>
															</div>
												...
												<button type="button" class="btn btn-danger btn-circle waves-effect waves-circle waves-float"
														onclick="Swal.fire({
															...
															confirmButtonText: 'Ya Hapus',
																...
																window.location = 'pasar-tradisional-del.php?id=<?php echo htmlentities($result['id_pasar_tradisional']);?>';
																}
															})">
	...
	<!-- Form Edit -->
	<?php include ('pasar-tradisional-input.php'); ?>
	<!-- #END# Form Edit -->
	<script type="text/javascript">
        function deleteall() {
			...
			confirmButtonText: 'Ya Hapus',
				...
				if (result.isConfirmed) {
				window.location = 'pasar-tradisional-delall.php';
				}
			})
		}
	</script>
