...
<body class="theme-red">
                        ...
                        <div class="header">
                            <?php 
								if(isset($_POST['cari'])){
									$dari = $_POST['dari'];
									$sampai = $_POST['sampai'];
									$sqlkat = "SELECT pasar_modern.*, tahun.* 
											   FROM pasar_modern, tahun WHERE 
											   pasar_modern.id_tahun=tahun.id_tahun AND 
											   tahun.tahun='$sampai'";
									$querykat = mysqli_query($koneksidb,$sqlkat);
									$tahun = mysqli_fetch_array($querykat);
							?>
							<h2>
                                Data Jumlah Pasar Modern di Kabupaten Tasikmalaya Tahun <?php echo $tahun['tahun']; ?>
                            </h2>
							<?php }else{ ?>
							<h2>
                                Data Jumlah Pasar Modern di Kabupaten Tasikmalaya
                            </h2>
							<?php }?>
                        </div>
                        <div class="body">
								...
								<table class="display table table-striped table-bordered table-hover" width="100%" id="example-table" cellspacing="0">
									<thead>
										<tr>
											...
											<th class="text-center">Minimarket (Unit)</th>
											<th class="text-center">Supermarket (Unit)</th>
									...
									<tbody>
										<?php 
											...
											if(isset($_POST['cari'])){
												$dari = $_POST['dari'];
												$sampai = $_POST['sampai'];
												$sqlthn = "SELECT pasar_modern.*, kecamatan.*, tahun.* FROM pasar_modern, kecamatan, tahun WHERE 
														   pasar_modern.id_kecamatan=kecamatan.id_kecamatan AND 
														   pasar_modern.id_tahun=tahun.id_tahun AND 
											...
											while ($result = mysqli_fetch_array($querythn)){
													$total += $result['minimarket'];
													$total2 += $result['supermarket'];
													$nomor++;
										?>
										<tr>
											<td class="text-center"><?php echo $nomor;?></td>
											<td><?php echo $result['kecamatan'];?></td>
											<td class="text-center"><?php echo htmlentities(number_format($result['minimarket'], 0, ",", "."));?></td>
											<td class="text-center"><?php echo htmlentities(number_format($result['supermarket'], 0, ",", "."));?></td>
											<td class="text-center">
												<button type="button" class="btn btn-warning btn-circle waves-effect waves-circle waves-float" 
														data-toggle="modal" data-target="#editModal<?php echo htmlentities($result['id_pasar_modern']);?>">
													<i class="material-icons">mode_edit</i>
												</button>&nbsp;&nbsp;
												<!-- Large Size -->
												<div class="modal fade" id="editModal<?php echo htmlentities($result['id_pasar_modern']);?>" tabindex="-1" role="dialog">
													<div class="modal-dialog modal-lg" role="document">
														<div class="modal-content">
															...
															<div class="modal-body">
																<form method="POST" action="module=pasar-modern-edit">
																	<div class="col-sm-6">
																		<div class="form-group">
																	...
																	<div class="col-sm-6">
																		<div class="form-group">
																			<label class="form-control-label" style="float:left;"><b>Jumlah Minimarket</b></label>
																			<div class="form-line">
																				<input type="hidden" name="id" value="<?php echo htmlentities($result['id_pasar_modern']);?>">
																				<input type="number" name="minimarket" class="form-control" min=0 value="<?php echo $result['minimarket'];?>">
																			</div>
																		</div>
																	</div>
																	<div class="col-sm-6">
																		<div class="form-group">
																			<label class="form-control-label" style="float:left;"><b>Jumlah Supermarket</b></label>
																			<div class="form-line">
																				<input type="number" name="supermarket" class="form-control" min=0 value="<?php echo $result['supermarket'];?>">
																			</div>
																		</div>
																	</div>
															</div>
												...
												<button type="button" class="btn btn-danger btn-circle waves-effect waves-circle waves-float"
														onclick="Swal.fire({
															...
															confirmButtonText: 'Ya Hapus',
																...
																window.location = 'pasar-modern-del.php?id=<?php echo htmlentities($result['id_pasar_modern']);?>';
																}
															})">
	...
	<!-- Form Edit -->
	<?php include ('pasar-modern-input.php'); ?>
	<!-- #END# Form Edit -->
	<script type="text/javascript">
        function deleteall() {
			...
			confirmButtonText: 'Ya Hapus',
				...
				if (result.isConfirmed) {
				window.location = 'pasar-modern-delall.php';
				}
			})
		}
	</script>
