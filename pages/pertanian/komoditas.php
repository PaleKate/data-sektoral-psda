...
<body class="theme-red">
                        ...
                        <div class="header">
                            <?php 
								if(isset($_POST['cari'])){
									$dari = $_POST['dari'];
									$sampai = $_POST['sampai'];
									$sqlkat = "SELECT komoditas.*, jenis_komoditas.*, tahun.* 
											   FROM komoditas, jenis_komoditas, tahun WHERE 
											   komoditas.id_jenis_komoditas=jenis_komoditas.id_jenis_komoditas AND 
											   komoditas.id_tahun=tahun.id_tahun AND 
											   tahun.tahun='$sampai' AND jenis_komoditas.jenis_komoditas='$dari'";
									$querykat = mysqli_query($koneksidb,$sqlkat);
									$tahun = mysqli_fetch_array($querykat);
							?>
							<h2>
                                Data Komoditas <?php echo $tahun['jenis_komoditas']; ?> di Kabupaten Tasikmalaya Tahun <?php echo $tahun['tahun']; ?>
                            </h2>
							<?php }else{ ?>
							<h2>
                                Data Komoditas di Kabupaten Tasikmalaya
                            </h2>
							<?php }?>
                        </div>
                        <div class="body">
										...
										<div class="col-md-2">
											<div class="form-group">
												<select class="form-control dari" name="dari" required>
													<option></option>
													<?php
														$mySql = "SELECT * FROM jenis_komoditas ORDER BY jenis_komoditas";
								...
								<table class="display table table-striped table-bordered table-hover" width="100%" id="example-table" cellspacing="0">
									<thead>
										<tr>
											...
											<th class="text-center">Provitas (kw/ha)</th>
									...
									<tbody>
										<?php 
											...
											if(isset($_POST['cari'])){
												$dari = $_POST['dari'];
												$sampai = $_POST['sampai'];
												$sqlthn = "SELECT komoditas.*, jenis_komoditas.*, kecamatan.*, tahun.* FROM komoditas, jenis_komoditas, kecamatan, tahun WHERE 
														   komoditas.id_kecamatan=kecamatan.id_kecamatan AND 
														   komoditas.id_jenis_komoditas=jenis_komoditas.id_jenis_komoditas AND 
														   komoditas.id_tahun=tahun.id_tahun AND 
														   tahun.tahun='$sampai' AND jenis_komoditas.id_jenis_komoditas='$dari' ORDER BY kecamatan.kecamatan ASC";
											}else{
											$sqlthn = "SELECT komoditas.*, jenis_komoditas.*, kecamatan.*, tahun.* FROM komoditas, jenis_komoditas, kecamatan, tahun WHERE 
													   komoditas.id_kecamatan=kecamatan.id_kecamatan AND 
													   komoditas.id_jenis_komoditas=jenis_komoditas.id_jenis_komoditas AND 
											...
											while ($result = mysqli_fetch_array($querythn)){
													$total += $result['luas_tanam'];
													$total2 += $result['luas_panen'];
													$total3 += $result['provitas'];
													$total4 += $result['produksi'];
													$nomor++;
										?>
										<tr>
											<td class="text-center"><?php echo $nomor;?></td>
											<td><?php echo $result['kecamatan'];?></td>
											<td class="text-center"><?php echo htmlentities(number_format($result['luas_tanam'], 0, ",", "."));?></td>
											<td class="text-center"><?php echo htmlentities(number_format($result['luas_panen'], 0, ",", "."));?></td>
											<td class="text-center"><?php echo htmlentities(number_format($result['provitas'], 0, ",", "."));?></td>
											...
											<td class="text-center">
												<button type="button" class="btn btn-warning btn-circle waves-effect waves-circle waves-float" 
														data-toggle="modal" data-target="#editModal<?php echo htmlentities($result['id_komoditas']);?>">
													<i class="material-icons">mode_edit</i>
												</button>&nbsp;&nbsp;
												<!-- Large Size -->
												<div class="modal fade" id="editModal<?php echo htmlentities($result['id_komoditas']);?>" tabindex="-1" role="dialog">
													<div class="modal-dialog modal-lg" role="document">
														<div class="modal-content">
															<div class="modal-header">
																<h4 class="modal-title" id="editModalLabel">Form Edit Data Komoditas <?php echo $result['jenis_komoditas']; ?></h4>
															</div>
															<div class="modal-body">
																<form method="POST" action="module=komoditas-edit">
																	<div class="col-sm-4">
																		<div class="form-group">
																			<label class="form-control-label" style="float:left;"><b>Jenis komoditas</b><span style="color:red">*</span></label>
																			<div style="text-align:left;">
																				<select class="form-control jenis_komoditas" name="id_jenis_komoditas" style="width:100%;" required>
																					<option value="<?php echo htmlentities($result['id_jenis_komoditas']);?>"><?php echo htmlentities($result['jenis_komoditas']);?></option>
																					<?php
																						$mySql = "SELECT * FROM jenis_komoditas ORDER BY jenis_komoditas";
																						$myQry = mysqli_query($koneksidb, $mySql);
																						while ($myData = mysqli_fetch_array($myQry)) {
																	...
																	<div class="col-sm-3">
																		<div class="form-group">
																			<label class="form-control-label" style="float:left;"><b>Luas Tanam (ha)</b></label>
																			<div class="form-line">
																				<input type="hidden" name="id" value="<?php echo htmlentities($result['id_komoditas']);?>">
																				<input type="number" name="luas_tanam" class="form-control" min=0 value="<?php echo $result['luas_tanam'];?>">
																			</div>
																		</div>
																	</div>
																	<div class="col-sm-3">
																		<div class="form-group">
																			<label class="form-control-label" style="float:left;"><b>Luas Panen (ha)</b></label>
																			<div class="form-line">
																				<input type="number" name="luas_panen" class="form-control" min=0 value="<?php echo $result['luas_panen'];?>">
																			</div>
																		</div>
																	</div>
																	<div class="col-sm-3">
																		<div class="form-group">
																			<label class="form-control-label" style="float:left;"><b>Provitas (kw/ha)</b></label>
																			<div class="form-line">
																				<input type="number" name="provitas" class="form-control" min=0 value="<?php echo $result['provitas'];?>">
																			</div>
																		</div>
																	</div>
																	<div class="col-sm-3">
																		<div class="form-group">
																			<label class="form-control-label" style="float:left;"><b>Produksi (ton)</b></label>
																			<div class="form-line">
												...
												<button type="button" class="btn btn-danger btn-circle waves-effect waves-circle waves-float"
														onclick="Swal.fire({
															...
															confirmButtonText: 'Ya Hapus',
																...
																window.location = 'komoditas-del.php?id=<?php echo htmlentities($result['id_komoditas']);?>';
																}
															})">
	...
	<!-- Form Edit -->
	<?php include ('komoditas-input.php'); ?>
	<!-- #END# Form Edit -->
	<script type="text/javascript">
        function deleteall() {
			...
			confirmButtonText: 'Ya Hapus',
				...
				if (result.isConfirmed) {
				window.location = 'komoditas-delall.php';
				}
			})
		}
	</script>
