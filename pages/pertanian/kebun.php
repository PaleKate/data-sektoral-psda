                        ...
                        <div class="header">
                            <?php 
								if(isset($_POST['cari'])){
									$dari = $_POST['dari'];
									$sampai = $_POST['sampai'];
									$sqlkat = "SELECT kebun.*, jenis_kebun.*, tahun.* 
											   FROM kebun, jenis_kebun, tahun WHERE 
											   kebun.id_jenis_kebun=jenis_kebun.id_jenis_kebun AND 
											   kebun.id_tahun=tahun.id_tahun AND 
											   tahun.tahun='$sampai' AND jenis_kebun.jenis_kebun='$dari'";
									$querykat = mysqli_query($koneksidb,$sqlkat);
									$tahun = mysqli_fetch_array($querykat);
							?>
							<h2>
                                Data Jumlah Panen dan Produksi Perkebunan <?php echo $tahun['jenis_kebun']; ?> di Kabupaten Tasikmalaya Tahun <?php echo $tahun['tahun']; ?>
                            </h2>
							<?php }else{ ?>
							<h2>
                                Data Jumlah Panen dan Produksi Perkebunan di Kabupaten Tasikmalaya
                            </h2>
							<?php }?>
                        </div>
                        <div class="body">
										...
										<div class="col-md-2">
											<div class="form-group">
												<select class="form-control dari" name="dari" required>
													<option></option>
													<?php
														$mySql = "SELECT * FROM jenis_kebun ORDER BY jenis_kebun";
								...
								<table class="display table table-striped table-bordered table-hover" width="100%" id="example-table" cellspacing="0">
									<thead>
										<tr>
											...
											<th class="text-center">Jumlah Panen (Pohon)</th>
											<th class="text-center">Jumlah Produksi (ton)</th>
									...
									<tbody>
										<?php 
											...
											if(isset($_POST['cari'])){
												$dari = $_POST['dari'];
												$sampai = $_POST['sampai'];
												$sqlthn = "SELECT kebun.*, jenis_kebun.*, kecamatan.*, tahun.* FROM kebun, jenis_kebun, kecamatan, tahun WHERE 
														   kebun.id_kecamatan=kecamatan.id_kecamatan AND 
														   kebun.id_jenis_kebun=jenis_kebun.id_jenis_kebun AND 
														   kebun.id_tahun=tahun.id_tahun AND 
														   tahun.tahun='$sampai' AND jenis_kebun.id_jenis_kebun='$dari' ORDER BY kecamatan.kecamatan ASC";
											}else{
											$sqlthn = "SELECT kebun.*, jenis_kebun.*, kecamatan.*, tahun.* FROM kebun, jenis_kebun, kecamatan, tahun WHERE 
													   kebun.id_kecamatan=kecamatan.id_kecamatan AND 
													   kebun.id_jenis_kebun=jenis_kebun.id_jenis_kebun AND 
											...
											while ($result = mysqli_fetch_array($querythn)){
													$total1 += $result['panen'];
													$total2 += $result['produksi'];
													$nomor++;
										?>
										<tr>
											<td class="text-center"><?php echo $nomor;?></td>
											<td><?php echo $result['kecamatan'];?></td>
											<td class="text-center"><?php echo htmlentities(number_format($result['panen'], 0, ",", "."));?></td>
											...
											<td class="text-center">
												<button type="button" class="btn btn-warning btn-circle waves-effect waves-circle waves-float" 
														data-toggle="modal" data-target="#editModal<?php echo htmlentities($result['id_kebun']);?>">
													<i class="material-icons">mode_edit</i>
												</button>&nbsp;&nbsp;
												<!-- Large Size -->
												<div class="modal fade" id="editModal<?php echo htmlentities($result['id_kebun']);?>" tabindex="-1" role="dialog">
													<div class="modal-dialog modal-lg" role="document">
														<div class="modal-content">
															<div class="modal-header">
																<h4 class="modal-title" id="editModalLabel">Form Edit Data kebun <?php echo $result['jenis_kebun']; ?></h4>
															</div>
															<div class="modal-body">
																<form method="POST" action="module=kebun-edit">
																	<div class="col-sm-4">
																		<div class="form-group">
																			<label class="form-control-label" style="float:left;"><b>Jenis kebun</b><span style="color:red">*</span></label>
																			<div style="text-align:left;">
																				<select class="form-control jenis_kebun" name="id_jenis_kebun" style="width:100%;" required>
																					<option value="<?php echo htmlentities($result['id_jenis_kebun']);?>"><?php echo htmlentities($result['jenis_kebun']);?></option>
																					<?php
																						$mySql = "SELECT * FROM jenis_kebun ORDER BY jenis_kebun";
																						$myQry = mysqli_query($koneksidb, $mySql);
																						while ($myData = mysqli_fetch_array($myQry)) {
																	...
																	<div class="col-sm-6">
																		<div class="form-group">
																			<label class="form-control-label" style="float:left;"><b>Jumlah Panen (Pohon)</b></label>
																			<div class="form-line">
																				<input type="hidden" name="id" value="<?php echo htmlentities($result['id_kebun']);?>">
																				<input type="number" name="panen" class="form-control" min=0 value="<?php echo $result['panen'];?>">
																			</div>
																		</div>
																	</div>
																	<div class="col-sm-6">
																		<div class="form-group">
																			<label class="form-control-label" style="float:left;"><b>Jumlah Produksi (ton)</b></label>
																			<div class="form-line">
												...
												<button type="button" class="btn btn-danger btn-circle waves-effect waves-circle waves-float"
														onclick="Swal.fire({
															...
															confirmButtonText: 'Ya Hapus',
																...
																window.location = 'kebun-del.php?id=<?php echo htmlentities($result['id_kebun']);?>';
																}
															})">
	...
	<script type="text/javascript">
        function deleteall() {
			...
			confirmButtonText: 'Ya Hapus',
				...
				if (result.isConfirmed) {
				window.location = 'kebun-delall.php';
				}
			})
		}
	</script>
