...
<body class="theme-red">
                        ...
                        <div class="header">
                            <?php 
								if(isset($_POST['cari'])){
									$dari = $_POST['dari'];
									$sampai = $_POST['sampai'];
									$sqlkat = "SELECT pemohon_kerja.*, tahun.* 
											   FROM pemohon_kerja, tahun WHERE 
											   pemohon_kerja.id_tahun=tahun.id_tahun AND 
											   tahun.tahun='$sampai'";
									$querykat = mysqli_query($koneksidb,$sqlkat);
									$tahun = mysqli_fetch_array($querykat);
							?>
							<h2>
                                Data Jumlah Pemohon Kerja (AK1) di Kabupaten Tasikmalaya Tahun <?php echo $tahun['tahun']; ?>
                            </h2>
							<?php }else{ ?>
							<h2>
                                Data Jumlah Pemohon Kerja (AK1) di Kabupaten Tasikmalaya
                            </h2>
							<?php }?>
                        </div>
                        <div class="body">
									...
									<tbody>
										<?php 
											...
											if(isset($_POST['cari'])){
												$dari = $_POST['dari'];
												$sampai = $_POST['sampai'];
												$sqlthn = "SELECT pemohon_kerja.*, kecamatan.*, tahun.* FROM pemohon_kerja, kecamatan, tahun WHERE 
														   pemohon_kerja.id_kecamatan=kecamatan.id_kecamatan AND 
														   pemohon_kerja.id_tahun=tahun.id_tahun AND 
											...
											while ($result = mysqli_fetch_array($querythn)){
													$total += $result['ak1_laki'];
													$total2 += $result['ak1_perempuan'];
													$nomor++;
										?>
										<tr>
											<td class="text-center"><?php echo $nomor;?></td>
											<td><?php echo $result['kecamatan'];?></td>
											<td class="text-center"><?php echo htmlentities(number_format($result['ak1_laki'], 0, ",", "."));?></td>
											<td class="text-center"><?php echo htmlentities(number_format($result['ak1_perempuan'], 0, ",", "."));?></td>
											<td class="text-center">
												<button type="button" class="btn btn-warning btn-circle waves-effect waves-circle waves-float" 
														data-toggle="modal" data-target="#editModal<?php echo htmlentities($result['id_pemohon_kerja']);?>">
													<i class="material-icons">mode_edit</i>
												</button>&nbsp;&nbsp;
												<!-- Large Size -->
												<div class="modal fade" id="editModal<?php echo htmlentities($result['id_pemohon_kerja']);?>" tabindex="-1" role="dialog">
													<div class="modal-dialog modal-lg" role="document">
														<div class="modal-content">
															<div class="modal-header">
																<h4 class="modal-title" id="editModalLabel">Form Edit Data Pemohon Kerja (AK1)</h4>
															</div>
															<div class="modal-body">
																<form method="POST" action="module=pemohon-kerja-edit">
																	<div class="col-sm-6">
																		<div class="form-group">
																	...
																	<div class="col-sm-6">
																		<div class="form-group">
																			<label class="form-control-label" style="float:left;"><b>Jumlah Pemohon Kerja Laki-laki</b></label>
																			<div class="form-line">
																				<input type="hidden" name="id" value="<?php echo htmlentities($result['id_pemohon_kerja']);?>">
																				<input type="number" name="ak1_laki" class="form-control" min=0 value="<?php echo $result['ak1_laki'];?>">
																			</div>
																		</div>
																	</div>
																	<div class="col-sm-6">
																		<div class="form-group">
																			<label class="form-control-label" style="float:left;"><b>Jumlah Pemohon Kerja Perempuan</b></label>
																			<div class="form-line">
																				<input type="number" name="ak1_perempuan" class="form-control" min=0 value="<?php echo $result['ak1_perempuan'];?>">
																			</div>
																		</div>
																	</div>
															</div>
												...
												<button type="button" class="btn btn-danger btn-circle waves-effect waves-circle waves-float"
														onclick="Swal.fire({
															...
															confirmButtonText: 'Ya Hapus',
																...
																window.location = 'pemohon-kerja-del.php?id=<?php echo htmlentities($result['id_pemohon_kerja']);?>';
																}
															})">
	...
	<!-- Form Edit -->
	<?php include ('pemohon-kerja-input.php'); ?>
	<!-- #END# Form Edit -->
	<script type="text/javascript">
        function deleteall() {
			...
			confirmButtonText: 'Ya Hapus',
				...
				if (result.isConfirmed) {
				window.location = 'pemohon-kerja-delall.php';
				}
			})
		}
	</script>
