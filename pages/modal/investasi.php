...
<body class="theme-red">
                        ...
                        <div class="header">
                            <?php 
								if(isset($_POST['cari'])){
									$dari = $_POST['dari'];
									$sampai = $_POST['sampai'];
									$sqlkat = "SELECT investasi.*, tahun.* 
											   FROM investasi, tahun WHERE 
											   investasi.id_tahun=tahun.id_tahun AND 
											   tahun.tahun='$sampai'";
									$querykat = mysqli_query($koneksidb,$sqlkat);
									$tahun = mysqli_fetch_array($querykat);
							?>
							<h2>
                                Data Jumlah Realisasi Investasi di Kabupaten Tasikmalaya Tahun <?php echo $tahun['tahun']; ?>
                            </h2>
							<?php }else{ ?>
							<h2>
                                Data Jumlah Realisasi Investasi di Kabupaten Tasikmalaya
                            </h2>
							<?php }?>
                        </div>
                        <div class="body">
								...
								<table class="display table table-striped table-bordered table-hover" width="100%" id="example-table" cellspacing="0">
									<thead>
										<tr>
											...
											<th class="text-center">Realisasi Investasi</th>
									...
									<tbody>
										<?php 
											...
											if(isset($_POST['cari'])){
												$dari = $_POST['dari'];
												$sampai = $_POST['sampai'];
												$sqlthn = "SELECT investasi.*, bulan.*, tahun.* FROM investasi, bulan, tahun WHERE 
														   investasi.id_bulan=bulan.id_bulan AND 
														   investasi.id_tahun=tahun.id_tahun AND 
														   tahun.tahun='$sampai' ORDER BY investasi.id_bulan ASC";
											}else{
											$sqlthn = "SELECT investasi.*, bulan.*, tahun.* FROM investasi, bulan, tahun WHERE 
													   investasi.id_bulan=bulan.id_bulan AND 
													   investasi.id_tahun=tahun.id_tahun AND 
											...
											while ($result = mysqli_fetch_array($querythn)){
													$total += $result['investasi'];
													$nomor++;
										?>
										<tr>
											<td class="text-center"><?php echo $nomor;?></td>
											<td><?php echo $result['bulan'];?></td>
											<td class="text-center"><?php echo htmlentities(number_format($result['investasi'], 0, ",", "."));?></td>
											<td class="text-center">
												<button type="button" class="btn btn-warning btn-circle waves-effect waves-circle waves-float" 
														data-toggle="modal" data-target="#editModal<?php echo htmlentities($result['id_investasi']);?>">
													<i class="material-icons">mode_edit</i>
												</button>&nbsp;&nbsp;
												<!-- Large Size -->
												<div class="modal fade" id="editModal<?php echo htmlentities($result['id_investasi']);?>" tabindex="-1" role="dialog">
													<div class="modal-dialog modal-lg" role="document">
														<div class="modal-content">
															<div class="modal-header">
																<h4 class="modal-title" id="editModalLabel">Form Edit Data Realisasi Investasi</h4>
															</div>
															<div class="modal-body">
																<form method="POST" action="module=investasi-edit">
																	<div class="col-sm-6">
																		<div class="form-group">
																	...
																	<div class="col-sm-12">
																		<div class="form-group">
																			<label class="form-control-label" style="float:left;"><b>Realisasi Investasi</b></label>
																			<div class="form-line">
																				<input type="hidden" name="id" value="<?php echo htmlentities($result['id_investasi']);?>">
																				<input type="number" name="investasi" class="form-control" min=0 value="<?php echo $result['investasi'];?>">
																			</div>
																		</div>
																	</div>
															</div>
												...
												<button type="button" class="btn btn-danger btn-circle waves-effect waves-circle waves-float"
														onclick="Swal.fire({
															...
															confirmButtonText: 'Ya Hapus',
																...
																window.location = 'investasi-del.php?id=<?php echo htmlentities($result['id_investasi']);?>';
																}
															})">
	...
	<!-- Form Edit -->
	<?php include ('investasi-input.php'); ?>
	<!-- #END# Form Edit -->
	<script type="text/javascript">
        function deleteall() {
			...
			confirmButtonText: 'Ya Hapus',
				...
				if (result.isConfirmed) {
				window.location = 'investasi-delall.php';
				}
			})
		}
	</script>
