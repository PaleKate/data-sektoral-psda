                        ...
                        <div class="header">
                            <?php 
								if(isset($_POST['cari'])){
									$dari = $_POST['dari'];
									$sampai = $_POST['sampai'];
									$sqlkat = "SELECT hotel.*, tahun.* 
											   FROM hotel, tahun WHERE 
											   hotel.id_tahun=tahun.id_tahun AND 
											   tahun.tahun='$sampai'";
									$querykat = mysqli_query($koneksidb,$sqlkat);
									$tahun = mysqli_fetch_array($querykat);
							?>
							<h2>
                                Data Jumlah Hotel dan Akomodasi Lainnya di Kabupaten Tasikmalaya Tahun <?php echo $tahun['tahun']; ?>
                            </h2>
							<?php }else{ ?>
							<h2>
                                Data Jumlah Hotel dan Akomodasi Lainnya di Kabupaten Tasikmalaya
                            </h2>
							<?php }?>
                        </div>
                        <div class="body">
								...
								<table class="display table table-striped table-bordered table-hover" width="100%" id="example-table" cellspacing="0">
									<thead>
										<tr>
											...
											<th class="text-center">Tempat Tidur</th>
									...
									<tbody>
										<?php 
											...
											if(isset($_POST['cari'])){
												$dari = $_POST['dari'];
												$sampai = $_POST['sampai'];
												$sqlthn = "SELECT hotel.*, kecamatan.*, tahun.* FROM hotel, kecamatan, tahun WHERE 
														   hotel.id_kecamatan=kecamatan.id_kecamatan AND 
														   hotel.id_tahun=tahun.id_tahun AND 
										...
										<tr>
											...
											<td class="text-center"><?php echo htmlentities(number_format($result['hotel'], 0, ",", "."));?></td>
											<td class="text-center"><?php echo htmlentities(number_format($result['kamar'], 0, ",", "."));?></td>
											<td class="text-center"><?php echo htmlentities(number_format($result['kasur'], 0, ",", "."));?></td>
											<td class="text-center">
												<button type="button" class="btn btn-warning btn-circle waves-effect waves-circle waves-float" 
														data-toggle="modal" data-target="#editModal<?php echo htmlentities($result['id_hotel']);?>">
													<i class="material-icons">mode_edit</i>
												</button>&nbsp;&nbsp;
												<!-- Large Size -->
												<div class="modal fade" id="editModal<?php echo htmlentities($result['id_hotel']);?>" tabindex="-1" role="dialog">
													<div class="modal-dialog modal-lg" role="document">
														<div class="modal-content">
															<div class="modal-header">
																<h4 class="modal-title" id="editModalLabel">Form Edit Data hotel</h4>
															</div>
															<div class="modal-body">
																<form method="POST" action="module=hotel-edit">
																	<div class="col-sm-6">
																		<div class="form-group">
																	...
																	<div class="col-sm-4">
																		<div class="form-group">
																			<label class="form-control-label" style="float:left;"><b>Jumlah Hotel</b></label>
																			<div class="form-line">
																				<input type="hidden" name="id" value="<?php echo htmlentities($result['id_hotel']);?>">
																				<input type="number" name="hotel" class="form-control" min=0 value="<?php echo $result['hotel'];?>">
																			</div>
																		</div>
																	</div>
																	<div class="col-sm-4">
																		<div class="form-group">
																			<label class="form-control-label" style="float:left;"><b>Jumlah Kamar</b></label>
																			<div class="form-line">
																				<input type="number" name="kamar" class="form-control" min=0 value="<?php echo $result['kamar'];?>">
																			</div>
																		</div>
																	</div>
																	<div class="col-sm-4">
																		<div class="form-group">
																			<label class="form-control-label" style="float:left;"><b>Jumlah Tempat Tidur</b></label>
																			<div class="form-line">
																				<input type="number" name="kasur" class="form-control" min=0 value="<?php echo $result['kasur'];?>">
																			</div>
																		</div>
																	</div>
															</div>
												...
												<button type="button" class="btn btn-danger btn-circle waves-effect waves-circle waves-float"
														onclick="Swal.fire({
															...
															confirmButtonText: 'Ya Hapus',
																...
																window.location = 'hotel-del.php?id=<?php echo htmlentities($result['id_hotel']);?>';
																}
															})">
	...
	<script type="text/javascript">
        function deleteall() {
			...
			confirmButtonText: 'Ya Hapus',
				...
				if (result.isConfirmed) {
				window.location = 'hotel-delall.php';
				}
			})
		}
	</script>
