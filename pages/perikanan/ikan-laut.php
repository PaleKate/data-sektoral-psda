...
<body class="theme-red">
                        ...
                        <div class="header">
                            <?php 
								if(isset($_POST['cari'])){
									$dari = $_POST['dari'];
									$sampai = $_POST['sampai'];
									$sqlkat = "SELECT ikan_laut.*, tahun.* 
											   FROM ikan_laut, tahun WHERE 
											   ikan_laut.id_tahun=tahun.id_tahun AND 
											   tahun.tahun='$sampai'";
									$querykat = mysqli_query($koneksidb,$sqlkat);
									$tahun = mysqli_fetch_array($querykat);
							?>
							<h2>
                                Data Jumlah Produksi dan Nilai Produksi Perikanan Laut di Kabupaten Tasikmalaya Tahun <?php echo $tahun['tahun']; ?>
                            </h2>
							<?php }else{ ?>
							<h2>
                                Data Jumlah Produksi dan Nilai Produksi Perikanan Laut di Kabupaten Tasikmalaya
                            </h2>
							<?php }?>
                        </div>
                        <div class="body">
								...
								<table class="display table table-striped table-bordered table-hover" width="100%" id="example-table" cellspacing="0">
									<thead>
										<tr>
											...
											<th class="text-center">Produksi Tongkol (ton)</th>
											<th class="text-center">Nilai Produksi Udang (Rp)</th>
											<th class="text-center">Nilai Produksi Tongkol (Rp)</th>
									...
									<tbody>
										<?php 
											...
											if(isset($_POST['cari'])){
												$dari = $_POST['dari'];
												$sampai = $_POST['sampai'];
												$sqlthn = "SELECT ikan_laut.*, kecamatan.*, tahun.* FROM ikan_laut, kecamatan, tahun WHERE 
														   ikan_laut.id_kecamatan=kecamatan.id_kecamatan AND
														   ikan_laut.id_tahun=tahun.id_tahun AND 
										...
										<tr>
											...
											<td class="text-center">
												<button type="button" class="btn btn-warning btn-circle waves-effect waves-circle waves-float" 
														data-toggle="modal" data-target="#editModal<?php echo htmlentities($result['id_ikan_laut']);?>">
													<i class="material-icons">mode_edit</i>
												</button>&nbsp;&nbsp;
												<!-- Large Size -->
												<div class="modal fade" id="editModal<?php echo htmlentities($result['id_ikan_laut']);?>" tabindex="-1" role="dialog">
													<div class="modal-dialog modal-lg" role="document">
														<div class="modal-content">
															<div class="modal-header">
																<h4 class="modal-title" id="editModalLabel">Form Edit Data Perikanan Laut</h4>
															</div>
															<div class="modal-body">
																<form method="POST" action="module=perikanan-laut-edit">
																	<div class="col-sm-6">
																		<div class="form-group">
																	...
																	<div class="col-sm-6">
																		<div class="form-group">
																			...
																			<div class="form-line">
																				<input type="hidden" name="id" value="<?php echo htmlentities($result['id_ikan_laut']);?>">
												...
												<button type="button" class="btn btn-danger btn-circle waves-effect waves-circle waves-float"
														onclick="Swal.fire({
															...
															confirmButtonText: 'Ya Hapus',
																...
																window.location = 'ikan-laut-del.php?id=<?php echo htmlentities($result['id_ikan_laut']);?>';
																}
															})">
	...
	<!-- Form Edit -->
	<?php include ('ikan-laut-input.php'); ?>
	<!-- #END# Form Edit -->
	<script type="text/javascript">
        function deleteall() {
			...
			confirmButtonText: 'Ya Hapus',
				...
				if (result.isConfirmed) {
				window.location = 'ikan-laut-delall.php';
				}
			})
		}
	</script>
