...
<body class="theme-red">
                        ...
                        <div class="header">
                            <?php 
								if(isset($_POST['cari'])){
									$dari = $_POST['dari'];
									$sampai = $_POST['sampai'];
									$sqlkat = "SELECT ikan_tangkap.*, tahun.* 
											   FROM ikan_tangkap, tahun WHERE 
											   ikan_tangkap.id_tahun=tahun.id_tahun AND 
											   tahun.tahun='$sampai'";
									$querykat = mysqli_query($koneksidb,$sqlkat);
									$tahun = mysqli_fetch_array($querykat);
							?>
							<h2>
                                Data Jumlah Produksi dan Nilai Produksi Perikanan Tangkap di Kabupaten Tasikmalaya Tahun <?php echo $tahun['tahun']; ?>
                            </h2>
							<?php }else{ ?>
							<h2>
                                Data Jumlah Produksi dan Nilai Produksi Perikanan Tangkap di Kabupaten Tasikmalaya
                            </h2>
							<?php }?>
                        </div>
                        <div class="body">
								...
								<table class="display table table-striped table-bordered table-hover" width="100%" id="example-table" cellspacing="0">
									<thead>
										<tr>
											...
											<th class="text-center">Produksi Perikanan Laut (ton)</th>
											<th class="text-center">Produksi Perikanan Umum (ton)</th>
											<th class="text-center">Nilai Produksi Perikanan Laut (Rp)</th>
											<th class="text-center">Nilai Produksi Perikanan Umum (Rp)</th>
									...
									<tbody>
										<?php 
											...
											if(isset($_POST['cari'])){
												$dari = $_POST['dari'];
												$sampai = $_POST['sampai'];
												$sqlthn = "SELECT ikan_tangkap.*, kecamatan.*, tahun.* FROM ikan_tangkap, kecamatan, tahun WHERE 
														   ikan_tangkap.id_kecamatan=kecamatan.id_kecamatan AND
														   ikan_tangkap.id_tahun=tahun.id_tahun AND 
											...
											while ($result = mysqli_fetch_array($querythn)){
													...
													$total2 += $result['produksi2'];
													$total3 += $result['nilai1'];
										...
										<tr>
											...
											<td class="text-center">
												<button type="button" class="btn btn-warning btn-circle waves-effect waves-circle waves-float" 
														data-toggle="modal" data-target="#editModal<?php echo htmlentities($result['id_ikan_tangkap']);?>">
													<i class="material-icons">mode_edit</i>
												</button>&nbsp;&nbsp;
												<!-- Large Size -->
												<div class="modal fade" id="editModal<?php echo htmlentities($result['id_ikan_tangkap']);?>" tabindex="-1" role="dialog">
													<div class="modal-dialog modal-lg" role="document">
														<div class="modal-content">
															<div class="modal-header">
																<h4 class="modal-title" id="editModalLabel">Form Edit Data Perikanan Tangkap</h4>
															</div>
															<div class="modal-body">
																<form method="POST" action="module=perikanan-tangkap-edit">
																	<div class="col-sm-6">
																		<div class="form-group">
																	...
																	<div class="col-sm-6">
																		<div class="form-group">
																			...
																			<div class="form-line">
																				<input type="hidden" name="id" value="<?php echo htmlentities($result['id_ikan_tangkap']);?>">
												...
												<button type="button" class="btn btn-danger btn-circle waves-effect waves-circle waves-float"
														onclick="Swal.fire({
															...
															confirmButtonText: 'Ya Hapus',
																...
																window.location = 'ikan-tangkap-del.php?id=<?php echo htmlentities($result['id_ikan_tangkap']);?>';
																}
															})">
	...
	<!-- Form Edit -->
	<?php include ('ikan-tangkap-input.php'); ?>
	<!-- #END# Form Edit -->
	<script type="text/javascript">
        function deleteall() {
			...
			confirmButtonText: 'Ya Hapus',
				...
				if (result.isConfirmed) {
				window.location = 'ikan-tangkap-delall.php';
				}
			})
		}
	</script>
